...
namespace MultinomialClassification {
    ...
    class DeepLearning {
        public static class Constants {
            public const double LearningRate = 0.01;   // 学習率
            ...
            public const int BatchSize = 100;           //バッチサイズ
            public const double L2Reg = 0.0001;          //重み減衰(L2正則化)
        ...
        public class NumberData {
            ...
            public NumberData(string[] data) {
                ...
                for (int i = 1; i < data.Length; i++) {
                    pixel_data[i] = double.Parse(data[i]) / 255;
                }
        ...
        static void Main(string[] args) {
            ...
            for (int i = 0; i < weightsW.Count; i++) {
                for (int j = 0; j < weightsW[i].Length; j++) {
                    weightsW[i][j] = (rng.NextDouble() * 2.0 - 1.0) * Math.Sqrt(6.0 / (Constants.InputSize));  // 小さなランダム値で初期化
                }
            }
            for (int i = 0; i < weightsV.Count; i++) {
                for (int j = 0; j < weightsV[i].Length; j++) {
                    weightsV[i][j] = (rng.NextDouble() * 2.0 - 1.0) * Math.Sqrt(6.0 / (Constants.HiddenLayerSize));
                }
            }
                ...
                foreach (var miniBatch in miniBatches) {
                    ...
                    foreach (var numberData in miniBatch) {
                        ...
                        for (int i = 0; i < b.Length; i++) {
                            ...
                            if (i == 0) {
                                // ダミー変数（バイアス項）の場合は勾配計算しない
                                bd[i] = 0.0; // バイアス項に対する勾配
                            }
                            else {
                                bd[i] = step(a[i - 1]) * SumYdW; // ReLUの勾配を考慮
                            }
                        }
                    ...
                    for (int i = 0; i < weightsW.Count; i++) {
                        for (int j = 0; j < weightsW[i].Length; j++) {
                            weightsW[i][j] -= (learningRate * gradientsW[i][j]) / miniBatch.Count+ 2 * Constants.L2Reg * weightsW[i][j];
                        }
                    }
                    for (int i = 0; i < weightsV.Count; i++) {
                        for (int j = 0; j < weightsV[i].Length; j++) {
                            weightsV[i][j] -= (learningRate * gradientsV[i][j]) / miniBatch.Count+ 2 * Constants.L2Reg * weightsV[i][j];
                        }
                    }
